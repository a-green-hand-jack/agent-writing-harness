...
Following the harness-engineering taxonomy of \citet{lee2026metaharness}, \MethodName{} is a \emph{research harness}: a stateful system that orchestrates interactions with LLMs by selecting the context, tools, and feedback presented to them during each stage of a research workflow.
...
\input{figures/05_pipeline}

\input{figures/06_wf2}

\input{figures/07_wf3}
...
These layers map to the three bottlenecks identified in \S\ref{sec:intro}: persistent \StateSymbol{}~(i) is realized by the per-project research wiki and versionable artifact contracts described in \S\ref{sec:wiki}; modular execution~(ii) is realized by self-contained Markdown skill files coordinated through the workflows of Figure~\ref{fig:pipeline}; and independent assurance~(iii) is realized by the assurance layer (\S\ref{sec:assurance}) under the cross-family executor/reviewer pairing detailed below.
