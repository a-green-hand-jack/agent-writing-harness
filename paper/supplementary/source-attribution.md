...
- Historical gate id: `external-source-migration` from the retired case harness
- Recorded by: automated case-harness-test run, case branch `case/arxiv-2605-03042`
- Recorded at: 2026-07-17
- Current control surface: `PAPER.md`, `EXPERIMENTS.md`, `PAPER_INTERFACES.md`, and this attribution record
...
| --- | --- | --- |
| `main.tex` preamble packages (`iclr2026_conference`, `inputenc`, `fontenc`, `hyperref`, `url`, `booktabs`, `amsfonts`, `amsmath`, `amsthm` + `\theoremstyle{remark}`/`\newtheorem{remark}`, `nicefrac`, `microtype`, `xcolor`, `graphicx`, `enumitem`, `multirow`, `tabularx`, `subcaption`, `natbib`, `fancyhdr`), `\iclrfinalcopy`, `\input{pixel_chars}`, and the `fancyhdr` setup | `paper/venue_preamble.tex` | Venue/class-bound setup. Pixel-art coordinates and the April 2026 header remain in this preamble. The title and visible author block now consume the stable interfaces in `paper/macros.tex` without changing their rendered content. |
| `\newcommand{\aris}`, `\newcommand{\skillmd}`, `\newcommand{\ariscode}` | `paper/macros.tex` | Source-compatible text shortcuts retained alongside the paper-first semantic interfaces. |
...
| `sec/6.related.tex` (`\section{Related Work}`, incl. `tab:comparison`) | `paper/sections/06_related.tex` | Imported section content. |
| `sec/2.overview.tex` (`\section{System Overview}`) + `sec/3.assurance.tex` (`\section{Cross-Model Assurance Stack}`) + `sec/4.realization.tex` (`\section{Implementation...}`) | `paper/sections/03_method.tex` | Three consecutive source sections share one canonical method file. Their `figure*` blocks use `\input{figures/NN_name}` wrappers; `tab:snapshot` and `tab:workflows` stay inline. |
| `sec/5.evidence.tex` intro + `\subsection{Ecosystem and Adoption}` (incl. `tab:ecosystem`, overnight-run paragraphs) | `paper/sections/04_exp.tex` | Imported deployment-evidence content; stable result interfaces preserve the rendered values and qualification. |
| `sec/5.evidence.tex` `\subsection{Limitations and Responsible Use}` | `paper/sections/05_limitations.tex` | Imported body, promoted to a standalone limitations section. |
| `sec/7.conclusion.tex` (`\section{Conclusion}`) | `paper/sections/07_conclusion.tex` | Imported conclusion content. |
...
separate semantic labels in the appendix. The source mapping above is the
durable provenance record; no duplicate figure registry is maintained.
...
numbers (0.4x, 2.5x, 5.0, and 7.5). The overnight score transition and its
qualitative scope limitation are now exposed through `\MainResult` and
`\MainResultUncertainty`; `EXPERIMENTS.md` records that no statistical
uncertainty was reported. Metadata literals such as 2026, GPT-5.4, and Workflow
1.5 remain identifiers, while small integer counts remain verbatim.
...
stops a metadata normalizer from parsing `\Adversarial` as a control sequence.
The same rendered title now uses the locked `\PaperTitle` interface. No wording
changed.
