...
`paper/` is the only authored source. Its scientific content is a verbatim migration of arXiv `2505.22954v3`. Claims, experiment interpretation, interface meaning, limitations, and canonical prose must not diverge by variant unless the Human explicitly approves a scientific revision in the canonical paper.
...
- verbatim scientific content of the migrated source;
- headline numbers and benchmark attribution;
- author identity and title;
- limitation and safety statements;
...
No active venue planning file exists under `.agents/knowledge/venues/`; the migrated source uses ICLR 2026 style files, and official current-year venue rules are **unresolved** and out of scope for this round.

## Overleaf working copy

The configured Overleaf project (`overleaf-dgm`, project `6a7cbcb091d988c7e64e85ec`) is a collaborative working copy of canonical `paper/`, not a second canonical source or a release instance. Its Git root contains only the tracked `paper/` tree. Repository governance, Agent tooling, CI, contracts, and release records are excluded.

Exports originate from a clean canonical branch (`main` or the protected `case/arxiv-2505-22954` branch). Online edits return through a dedicated `sync/overleaf-*` branch and must pass the ordinary paper build and repository verification before merge. A detected online edit blocks outbound replacement until reviewed and imported.
...
No Human-approved release instance exists for this case. The Round-1 legacy committed `release/` tree was retired into `releases/records/arxiv-2505-22954-legacy-round1.md` during template adoption. Published revisions use a new ID; do not edit an old record to represent new artifacts.
