...
""",
    ...
    def test_dependency_boundary_allows_locked_natbib_macro_input(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            fixture(root)
            write(
...
""",
    ...
    def test_dependency_boundary_rejects_same_macro_input_outside_natbib(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            fixture(root)
            write(root / "paper/style/other.sty", "\\input{#2}\n")
            result = run(root)
            self.assertNotEqual(result.returncode, 0)
            self.assertIn("unresolved dynamic input dependency", result.stdout)
