...
""",
    ...
    def test_dependency_boundary_allows_locked_coral_graphics_path(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            fixture(root)
    ...
    def test_dependency_boundary_rejects_graphics_path_outside_coral_preamble(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            fixture(root)
            write(root / "paper/style/other.sty", "\\graphicspath{{figures/}}\n")
            result = run(root)
            self.assertNotEqual(result.returncode, 0)
            self.assertIn("unsupported TeX dependency search path directive", result.stdout)
