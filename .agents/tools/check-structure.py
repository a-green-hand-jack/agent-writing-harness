...
PATH_DIRECTIVE_ALLOWLIST = {
    ("paper/venue_preamble.tex", r"\graphicspath{{figures/}}"),
}
...
def check_dependency_boundary(root: Path, profile: dict[str, object]) -> int:
    ...
    for path in source_files:
        ...
        for match in UNSUPPORTED_PATH_DIRECTIVE_RE.finditer(text):
            allowed = any(
                relative.as_posix() == allowed_path
                and text.startswith(allowed_directive, match.start())
