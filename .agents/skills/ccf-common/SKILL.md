...
- Resources: any existing sibling `references/`, `scripts/`, `resources/`, `templates/`, or `assets/` directories under `.agents/vendor/ccfa-skills/ccf-common`.
...
The vendored `artifact-contracts.md` canonical paths (reviews/, submission/, artifact/, experiments/results.*) are advisory and template-foreign; any generated artifact path must follow the template contracts: `references/ledger.json`, `PUBLICATION.md` delivery targets, and ignored `.agents/runtime/` for candidates. ccfa.yaml must not be created.
